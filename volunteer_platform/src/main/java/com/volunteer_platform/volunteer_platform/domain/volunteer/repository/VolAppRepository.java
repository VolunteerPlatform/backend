...
public interface VolAppRepository extends JpaRepository<AppHistory, Long> {
    ...
    @Query("select m from AppHistory m join fetch m.volActivityTime v join fetch m.member mm join fetch mm.memberInfo join fetch mm.member1365Info where v.volActivity.id = ?1 and m.isAuthorized = ?2 and v.activityDate = ?3")
